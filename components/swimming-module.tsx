...
import { Droplets, Search, Check, AlertCircle, X, Award, ChevronRight, Filter } from 'lucide-react';
...
// Data structures
type CapLevel = 'orange' | 'red' | 'green' | 'lightBlue' | 'darkBlue' | 'black';

type EvaluationStatus = 'achieved' | 'developing' | 'untested';
...
interface Student {
  ...
  age: number;
  level: CapLevel;
  lastEvaluation: string; // Date string
  evalDetails: Evaluation;
}

const levels: Record<CapLevel, { name: string; colorClass: string; bgClass: string }> = {
                      ...
                      {([
                        ...
                        <div key={criterion.key} className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-amber-200">
                          ...
                          <div className="flex flex-wrap gap-2 shrink-0">
                            <EvaluationButton
                              status={selectedStudent.evalDetails[criterion.key]}
                              value="achieved"
                              label="Atingido"
                              colorClass="bg-emerald-500 text-white"
                              onClick={() => handleUpdateEvaluation(criterion.key, 'achieved')}
                            />
                            <EvaluationButton
                              status={selectedStudent.evalDetails[criterion.key]}
                              value="developing"
                              ...
                              onClick={() => handleUpdateEvaluation(criterion.key, 'developing')}
                            />
                            <EvaluationButton
                              status={selectedStudent.evalDetails[criterion.key]}
                              value="untested"
                              ...
                              onClick={() => handleUpdateEvaluation(criterion.key, 'untested')}
                            />
                          </div>
                        </div>
                      ))}
