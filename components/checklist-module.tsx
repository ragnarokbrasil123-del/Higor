...
import { CheckSquare, Plus, Trash2, Edit2, Check, X, ClipboardList, ChevronLeft, ChevronRight, CalendarDays, Camera } from 'lucide-react';
...
const defaultTasks = [
  "Área Externa da Piscina", "Deck da Piscina", "Banheiro Feminino Piscina", 
  "Banheiro Masculino Piscina", "Vestiário Feminino Piscina", "Vestiário Masculino Piscina", 
  "Casa de Máquinas da Piscina", "Sala de Ginástica", "Sala de Dança", "Sala de Lutas", 
  "Sala de Pilates", "Sala de Avaliação Física", "Recepção", "Sala de Espera", "Banheiros Sociais"
];
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 mb-6 mt-4">
          <div className="text-white">
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-1 md:mb-2 flex items-center gap-2 md:gap-3">
              <ClipboardList className="w-6 h-6 md:w-8 md:h-8 text-amber-400" />
              Checklist Limpeza
            </h1>
            ...
            <div className="flex items-center gap-1 md:gap-2 mt-4">
              <button onClick={() => changeDate(-1)} className="p-2 hover:bg-white/10 rounded-lg text-amber-400"><ChevronLeft className="w-5 h-5" /></button>
              <div className="flex items-center gap-2 md:gap-3 px-3 md:px-5 py-2 bg-white/10 backdrop-blur-md rounded-xl border border-white/10">
                <CalendarDays className="w-4 h-4 md:w-5 md:h-5 text-amber-400" />
                <div className="flex flex-col">
                  {isToday() && <span className="text-[9px] uppercase font-bold text-amber-400 tracking-wider">Hoje</span>}
              ...
              <button onClick={() => changeDate(1)} className="p-2 hover:bg-white/10 rounded-lg text-amber-400"><ChevronRight className="w-5 h-5" /></button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden border border-slate-100">
          <div className="p-3 md:p-6 border-b border-slate-100 bg-slate-50/50">
            <form onSubmit={handleAddTask} className="flex flex-col md:flex-row gap-2">
              ...
              <button type="submit" disabled={!newTaskTitle.trim()} className="px-6 py-3 bg-amber-400 text-black rounded-xl font-bold text-sm disabled:opacity-50 active:scale-95 transition-all">Adicionar</button>
            </form>
          </div>
