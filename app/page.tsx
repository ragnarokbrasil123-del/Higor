...
import { Droplets, ClipboardList, UserPlus, ShieldCheck, LogOut } from 'lucide-react';
...
type Tab = 'swimming' | 'cleaning' | 'registration' | 'team';
...
export default function Page() {
  ...
  // 2. SE FOR O PAI/ALUNO -> MOSTRA APENAS O PORTAL DE LEITURA
  if (user.role === 'client') {
    return <ClientPortal student={user.data} onLogout={() => setUser(null)} />;
  }

  // 3. SE FOR A EQUIPE -> MOSTRA O PAINEL DE CONTROLE
    ...
    <div className="fixed inset-0 flex flex-col md:flex-row bg-[#F0F4F8] overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-black text-white flex-col shrink-0 border-r border-slate-800 z-20">
        ...
        <nav className="mt-6 flex-1 space-y-2 px-4 flex flex-col pb-6">
          ...
          {/* SÓ O ADMIN (DONO) VÊ ESSES DOIS BOTÕES */}
          {isAdmin && (
      ...
      <header className="md:hidden bg-black flex flex-col shrink-0 z-20 shadow-md border-b border-slate-900">
        <div className="h-14 flex justify-between items-center px-4">
          <div className="w-8" /> {/* Espaçador para centralizar a logo */}
        ...
        <div className="flex justify-between items-center px-2 pb-3 gap-2 bg-black overflow-x-auto custom-scrollbar">
          <button onClick={() => setActiveTab('registration')} className={cn("flex-1 min-w-[75px] py-2.5 rounded-xl flex flex-col items-center justify-center gap-1 transition-all border", activeTab === 'registration' ? "bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-500/20" : "bg-slate-900 text-slate-400 border-slate-800")}>
            <UserPlus className="w-4 h-4" />
            <span className="text-[10px] font-bold tracking-wide">Cadastro</span>
          </button>
          
          <button onClick={() => setActiveTab('swimming')} className={cn("flex-1 min-w-[75px] py-2.5 rounded-xl flex flex-col items-center justify-center gap-1 transition-all border", activeTab === 'swimming' ? "bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-500/20" : "bg-slate-900 text-slate-400 border-slate-800")}>
            <Droplets className="w-4 h-4" />
            <span className="text-[10px] font-bold tracking-wide">Natação</span>
          </button>

          {isAdmin && (
            <button onClick={() => setActiveTab('cleaning')} className={cn("flex-1 min-w-[75px] py-2.5 rounded-xl flex flex-col items-center justify-center gap-1 transition-all border", activeTab === 'cleaning' ? "bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-500/20" : "bg-slate-900 text-slate-400 border-slate-800")}>
          ...
          {isAdmin && (
            <button onClick={() => setActiveTab('team')} className={cn("flex-1 min-w-[75px] py-2.5 rounded-xl flex flex-col items-center justify-center gap-1 transition-all border", activeTab === 'team' ? "bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-500/20" : "bg-slate-900 text-slate-400 border-slate-800")}>
